/*
 * This file is generated by jOOQ.
 */
package generated.sky.regular.income.tables.records;


import generated.sky.regular.income.tables.FinancialTransaction;

import java.sql.Date;
import java.util.UUID;

import javax.annotation.Generated;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record7;
import org.jooq.Row7;
import org.jooq.impl.UpdatableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.11.11"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class FinancialTransactionRecord extends UpdatableRecordImpl<FinancialTransactionRecord> implements Record7<UUID, UUID, Date, Integer, Boolean, String, String> {

    private static final long serialVersionUID = 1334840634;

    /**
     * Setter for <code>REGULAR_INCOME.financial_transaction.id</code>.
     */
    public FinancialTransactionRecord setId(UUID value) {
        set(0, value);
        return this;
    }

    /**
     * Getter for <code>REGULAR_INCOME.financial_transaction.id</code>.
     */
    public UUID getId() {
        return (UUID) get(0);
    }

    /**
     * Setter for <code>REGULAR_INCOME.financial_transaction.bank_statement_id</code>.
     */
    public FinancialTransactionRecord setBankStatementId(UUID value) {
        set(1, value);
        return this;
    }

    /**
     * Getter for <code>REGULAR_INCOME.financial_transaction.bank_statement_id</code>.
     */
    public UUID getBankStatementId() {
        return (UUID) get(1);
    }

    /**
     * Setter for <code>REGULAR_INCOME.financial_transaction.date_record</code>.
     */
    public FinancialTransactionRecord setDateRecord(Date value) {
        set(2, value);
        return this;
    }

    /**
     * Getter for <code>REGULAR_INCOME.financial_transaction.date_record</code>.
     */
    public Date getDateRecord() {
        return (Date) get(2);
    }

    /**
     * Setter for <code>REGULAR_INCOME.financial_transaction.amount_value_cents</code>.
     */
    public FinancialTransactionRecord setAmountValueCents(Integer value) {
        set(3, value);
        return this;
    }

    /**
     * Getter for <code>REGULAR_INCOME.financial_transaction.amount_value_cents</code>.
     */
    public Integer getAmountValueCents() {
        return (Integer) get(3);
    }

    /**
     * Setter for <code>REGULAR_INCOME.financial_transaction.is_periodic</code>.
     */
    public FinancialTransactionRecord setIsPeriodic(Boolean value) {
        set(4, value);
        return this;
    }

    /**
     * Getter for <code>REGULAR_INCOME.financial_transaction.is_periodic</code>.
     */
    public Boolean getIsPeriodic() {
        return (Boolean) get(4);
    }

    /**
     * Setter for <code>REGULAR_INCOME.financial_transaction.reason</code>.
     */
    public FinancialTransactionRecord setReason(String value) {
        set(5, value);
        return this;
    }

    /**
     * Getter for <code>REGULAR_INCOME.financial_transaction.reason</code>.
     */
    public String getReason() {
        return (String) get(5);
    }

    /**
     * Setter for <code>REGULAR_INCOME.financial_transaction.checksum</code>.
     */
    public FinancialTransactionRecord setChecksum(String value) {
        set(6, value);
        return this;
    }

    /**
     * Getter for <code>REGULAR_INCOME.financial_transaction.checksum</code>.
     */
    public String getChecksum() {
        return (String) get(6);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    /**
     * {@inheritDoc}
     */
    @Override
    public Record1<UUID> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record7 type implementation
    // -------------------------------------------------------------------------

    /**
     * {@inheritDoc}
     */
    @Override
    public Row7<UUID, UUID, Date, Integer, Boolean, String, String> fieldsRow() {
        return (Row7) super.fieldsRow();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Row7<UUID, UUID, Date, Integer, Boolean, String, String> valuesRow() {
        return (Row7) super.valuesRow();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<UUID> field1() {
        return FinancialTransaction.FINANCIAL_TRANSACTION.ID;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<UUID> field2() {
        return FinancialTransaction.FINANCIAL_TRANSACTION.BANK_STATEMENT_ID;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Date> field3() {
        return FinancialTransaction.FINANCIAL_TRANSACTION.DATE_RECORD;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Integer> field4() {
        return FinancialTransaction.FINANCIAL_TRANSACTION.AMOUNT_VALUE_CENTS;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Boolean> field5() {
        return FinancialTransaction.FINANCIAL_TRANSACTION.IS_PERIODIC;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field6() {
        return FinancialTransaction.FINANCIAL_TRANSACTION.REASON;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field7() {
        return FinancialTransaction.FINANCIAL_TRANSACTION.CHECKSUM;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UUID component1() {
        return getId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UUID component2() {
        return getBankStatementId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Date component3() {
        return getDateRecord();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Integer component4() {
        return getAmountValueCents();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean component5() {
        return getIsPeriodic();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component6() {
        return getReason();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component7() {
        return getChecksum();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UUID value1() {
        return getId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public UUID value2() {
        return getBankStatementId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Date value3() {
        return getDateRecord();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Integer value4() {
        return getAmountValueCents();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean value5() {
        return getIsPeriodic();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value6() {
        return getReason();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value7() {
        return getChecksum();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public FinancialTransactionRecord value1(UUID value) {
        setId(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public FinancialTransactionRecord value2(UUID value) {
        setBankStatementId(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public FinancialTransactionRecord value3(Date value) {
        setDateRecord(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public FinancialTransactionRecord value4(Integer value) {
        setAmountValueCents(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public FinancialTransactionRecord value5(Boolean value) {
        setIsPeriodic(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public FinancialTransactionRecord value6(String value) {
        setReason(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public FinancialTransactionRecord value7(String value) {
        setChecksum(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public FinancialTransactionRecord values(UUID value1, UUID value2, Date value3, Integer value4, Boolean value5, String value6, String value7) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached FinancialTransactionRecord
     */
    public FinancialTransactionRecord() {
        super(FinancialTransaction.FINANCIAL_TRANSACTION);
    }

    /**
     * Create a detached, initialised FinancialTransactionRecord
     */
    public FinancialTransactionRecord(UUID id, UUID bankStatementId, Date dateRecord, Integer amountValueCents, Boolean isPeriodic, String reason, String checksum) {
        super(FinancialTransaction.FINANCIAL_TRANSACTION);

        set(0, id);
        set(1, bankStatementId);
        set(2, dateRecord);
        set(3, amountValueCents);
        set(4, isPeriodic);
        set(5, reason);
        set(6, checksum);
    }
}
